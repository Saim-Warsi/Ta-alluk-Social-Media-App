        ...
        <div className='w-screen sm:w-[calc(100vw-240px)] lg:max-w-2xl no-scrollbar overflow-x-auto px-4'>
            <div className="flex gap-4 pb-5">
                ...
                {stories.map((story, index) => (
                    <div key={index} onClick={()=>setViewStory(story)} className='relative rounded-lg shadow min-w-30 max-w-30 max-h-40 overflow-hidden cursor-pointer hover:shadow-lg transition-all duration-200 bg-yellow-500 hover:bg-amber-500 active:scale-95'>
                        <img 
                            src={story.user.profile_picture} 
                            alt={story.user.username}
                            className='absolute size-8 top-3 left-3 z-10 rounded-full ring ring-gray-100 shadow'
                        />
                        <p className='absolute top-12 left-3 text-white/60 text-sm truncate max-w-24 z-10'>{story.content}</p>
