        ...
        <div className="mt-6">
          <div className="bg-white rounded-full shadow p-1 flex max-w-md mx-auto ">
            {['posts','media','likes'].map((tab)=>(
              <button onClick={()=> setActiveTab(tab)} key={tab} className={`flex-1 px-4 py-2 text-sm font-medium rounded-full transition-colors cursor-pointer ${activeTab === tab ? "bg-yellow-500 text-white" : "text-gray-600 hover:text-gray-900"}`}>
              ...
              <div className="flex flex-wrap mt-6 max-w-6xl ">
                  {
                    ...
                    {post.image_urls.map((img, index)=>(
                      ...
                      <p
                      className='absolute bottom-0 right-0 text-xs p-1 px-3 backdrop-blur-lg bg-yellow-500/40 text-white opacity-0 group-hover:opacity-100 transition duration-300'
