...
// update user data from their id
export const updateUserData = async (req, res) => {
  try {
    ...
    if(tempUser.username !== username){
        ...
        if(user){
            //test to check if the username is already used
...
// find users using username, location, name and email
...
export const followUser = async (req, res) => {
  try {
    ...
    await toUser.save()
    res.json({success:true, message: "Started following"})
...
export const sendConnectionRequest = async(req,res)=>{
  try{
      ...
      // limit user to not send more than 20 requests per day
      let last24Hours = new Date(Date.now() - 24 * 60 * 60 * 1000);
      const connectionRequests = await Connection.find({from_user_id: userId, created_at: {$gt: last24Hours}}) 
      if(connectionRequests.length >= 20){
        return res.json({success: false, message: "Please wait 24hrs before building more ta'alluks."})
      }
